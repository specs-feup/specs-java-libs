 ...
 * which helps the calling method communicate to <i>its</i> caller that <i>that</i> caller has made a mistake. Example:
 * 
 * <pre>
 * {@code
 * 
 ...
 * {@code exampleBadCaller} made an error in <i>its</i> call to {@code sqrt}.
 *
 * <h3>Warning about performance</h3>
 *
 * <p>
 * The goal of this class is to improve readability of code, but in some circumstances this may come at a significant
 * performance cost. Remember that parameter values for message construction must all be computed eagerly, and
 * autoboxing and varargs array creation may happen as well, even when the precondition check then succeeds (as it
 * should almost always do in production). In some circumstances these wasted CPU cycles and allocations can add up to a
 * real problem. Performance-sensitive precondition checks can always be converted to the customary form:
 * 
 * <pre>
 * {@code
 * 
 ...
 *
 * <p>
 * Not every type of precondition failure is supported by these methods. Continue to throw standard JDK exceptions such
 * as {@link java.util.NoSuchElementException} or {@link UnsupportedOperationException} in the situations they are
 * intended for.
 *
 * <h3>Non-preconditions</h3>
 *
 * <p>
 * It is of course possible to use the methods of this class to check for invalid conditions which are <i>not the
 * caller's fault</i>. Doing so is <b>not recommended</b> because it is misleading to future readers of the code and of
 * stack traces. See <a href="http://code.google.com/p/guava-libraries/wiki/ConditionalFailuresExplained">Conditional
 ...
 * {@link java.util.Objects#requireNonNull(Object)}. Instead, use whichever of {@link #checkNotNull(Object)} or
 * {@link Verify#verifyNotNull(Object)} is appropriate to the situation. (The same goes for the message-accepting
 * overloads.)
 *
 * <h3>Only {@code %s} is supported</h3>
 *
 * <p>
 * In {@code Preconditions} error message template strings, only the {@code "%s"} specifier is supported, not the full
 * range of {@link java.util.Formatter} specifiers. However, note that if the number of arguments does not match the
 * number of occurrences of {@code "%s"} in the format string, {@code Preconditions} will still behave as expected, and
