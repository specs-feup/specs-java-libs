...
@FunctionalInterface
public interface EnumLoggerOld<T extends Enum<T>> {
    ...
    default void setLevelAll(Level level) {
        for (T enumConst : getEnumClass().getEnumConstants()) {
            setLevel(enumConst, level);
        }
    ...
    default String getLoggerName(T tag) {
        String loggerName = getEnumClass().getName() + ".";
    ...
    default LoggerWrapper getBaseLogger() {
        return SpecsLoggers.getLogger(getEnumClass().getName());
    }
    ...
    static <T extends Enum<T>> EnumLoggerOld<T> newInstance(Class<T> enumClass) {
        return () -> enumClass;
    }
