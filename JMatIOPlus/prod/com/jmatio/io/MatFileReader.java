...
public class MatFileReader {
    // TODO: Consider disabling MEMORY_MAPPED_FILE, due to sun.misc.Cleaner, and use DIRECT_BYTE_BUFFER instead
    ...
    private static final int getDefaultReader() {
        // Memory mapped file not supported in Java 9 and greater
    ...
    /**
     * Creates instance of <code>MatFileReader</code> and reads MAT-file from location given as <code>fileName</code>.
     * 
     ...
     * Creates instance of <code>MatFileReader</code> and reads MAT-file from location given as <code>fileName</code>.
     * 
     * Results are filtered by <code>MatFileFilter</code>. Arrays that do not meet filter match condition will not be
     ...
     * available in results.
     * <p>
     * <i>Note: this method reads file using the memory mapped file policy, see notes to </code>
     * {@link #read(File, MatFileFilter, com.jmatio.io.MatFileReader.MallocPolicy)}</code>
     * 
     * @param file
     *            the MAT-file
     * @param MatFileFilter
    ...
    public MatFileReader(File file, MatFileFilter filter) throws IOException {
        ...
        read(file, filter, getDefaultReader());
    }
     ...
     * <p>
     * This method calls <code>read(file, new MatFileFilter(), MallocPolicy.MEMORY_MAPPED_FILE)</code>.
     * 
     * @param file
    ...
    public synchronized Map<String, MLArray> read(File file) throws IOException {
        return read(file, new MatFileFilter(), getDefaultReader());
    }
     ...
     * Reads the content of a MAT-file and returns the mapped content.
     * <p>
     * Because of java bug <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4724038">#4724038</a> which
     * disables releasing the memory mapped resource, additional different allocation modes are available.
     * <ul>
     * <li><code>{@link #MEMORY_MAPPED_FILE}</code> - a memory mapped file</li>
     * <li><code>{@link #DIRECT_BYTE_BUFFER}</code> - a uses <code>{@link ByteBuffer#allocateDirect(int)}</code> method
     * to read in the file contents</li>
     * <li><code>{@link #HEAP_BYTE_BUFFER}</code> - a uses <code>{@link ByteBuffer#allocate(int)}</code> method to read
     * in the file contents</li>
     * </ul>
     * <i>Note: memory mapped file will try to invoke a nasty code to relase it's resources</i>
     * 
     * @param file
     *            a valid MAT-file file to be read
     * @param filter
    ...
    public synchronized Map<String, MLArray> read(File file, MatFileFilter filter, int policy) throws IOException {
        ...
        try {
            ...
            if (buf.remaining() != 0) {
                SpecsLogs.warn("Buffer is not empty. Check if there is any problem. Contents:" + buf.toString());
            }
    ...
    /**
     * Workaround taken from bug <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4724038">#4724038</a> to
     * release the memory mapped byte buffer.
     * <p>
     * Little quote from SUN: <i>This is highly inadvisable, to put it mildly. It is exceedingly dangerous to forcibly
     * unmap a mapped byte buffer that's visible to Java code. Doing so risks both the security and stability of the
     * system</i>
     * <p>
     * Since the memory byte buffer used to map the file is not exposed to the outside world, maybe it's save to use it
     * without being cursed by the SUN. Since there is no other solution this will do (don't trust voodoo GC invocation)
     * 
     * @param buffer
        ...
        AccessController.doPrivileged(new PrivilegedAction<Object>() {
            ...
            public Object run() {
                try {
                    ...
                    double javaVersion = SpecsSystem.getJavaVersionNumber();

                    // Using reflection so that both code paths work on all Java versions

                    // Java 9 and higher path
                    if (javaVersion > 1.8) {
                        throw new RuntimeException("Not supported in Java 9 or greater");
                        /*
                        // Branching
                        // Java 9 alternative
                        SpecsLogs.warn("DEBUG: Using new cleaning method");
                        // Class<?> cleanerClass = Class.forName("java.lang.ref.Cleaner");
                        // Method cleanerCreate = cleanerClass.getMethod("create");
                        //
                        ...
                        // };
                        //
                        // Object cleanableObject = registerMethod.invoke(cleanerObject, buffer, action);
                        // Method cleanableClean = cleanableObject.getClass().getMethod("clean");
                        // cleanableClean.invoke(cleanableObject);
                        Runnable action = () -> {
                            try {
                                getCleanerMethod.invoke(buffer, new Object[0]);
                            } catch (IllegalAccessException | IllegalArgumentException
                        ...
                        // Cleaner cleaner = (Cleaner) cleanerCreate.invoke(null);
                        Object cleanerObject = cleanerCreate.invoke(null);
                        
                        Method registerMethod = cleanerClass.getMethod("register", Object.class, Runnable.class);
                        
                        // Cleanable cleanable = cleaner.register(buffer, action);
                        // Cleanable cleanable = (Cleanable) registerMethod.invoke(cleanerObject, buffer, action);
                        Object cleanableObject = registerMethod.invoke(cleanerObject, buffer, action);
                        
                        // Class<?> cleanableClass = Class.forName("java.lang.ref.Cleaner.Cleanable");
                        Class<?> cleanableClass = cleanableObject.getClass();
                        // System.out.println("CLASS:" + cleanableClass);
                        Method cleanableClean = cleanableClass.getMethod("clean");
                        cleanableClean.setAccessible(true);
                        cleanableClean.invoke(cleanableObject);
                        // cleanable.clean();
                    ...
                    // SpecsLogs.msgInfo("DEBUG: Using old cleaning method");
                    Object cleanerObject = getCleanerMethod.invoke(buffer, new Object[0]);
                    Method cleanerClean = cleanerObject.getClass().getMethod("clean");
                    cleanerClean.invoke(cleanerObject);
     ...
     * @deprecated use <code>getContent</code> which returns a Map to provide easier access to <code>MLArray</code>s
     *             contained in MAT-file
     */
    @Deprecated
     ...
     * @param -
     *            array name
     * @return - the <code>MLArray</code> to which this file maps the specified name, or null if the file contains no
    ...
    /**
     * Decompresses (inflates) bytes from input stream. Stream marker is being set at +<code>numOfBytes</code> position
     * of the stream.
     * 
     * @param is
     *            - input byte buffer
    ...
    private ByteBuffer inflate(final ByteBuffer buf, final int numOfBytes) throws IOException {
        ...
        int i;
        try (_ByteArrayOutputStream baos = new _ByteArrayOutputStream(numOfBytes)) {
            do {
    ...
    /**
     * Reads data form byte buffer. Searches for either <code>miCOMPRESSED</code> data or <code>miMATRIX</code> data.
     * 
     * Compressed data are inflated and the product is recursively passed back to this same method.
     * 
     ...
     * 
     * If reading was not finished (which is normal for filtered results) returns <code>null</code>.
     * 
     ...
     *            - input byte buffer
     * @param isRoot
     *            - when <code>true</code> informs that if this is a top level matrix
     ...
     *             if reading from buffer fails
     */
    private static String readName(ByteBuffer buf) throws IOException {
        String s;
     ...
     *            <code>ByteBuffer</code>
     * @throws IOException
     *             if reading from buffer fails or if this is not a valid MAT-file
     */
